...
const FloatingNav: React.FC = () => {
  ...
  const navItems = React.useMemo(() => [
    ...
    { name: t('nav.contacts'), href: '#contacts', icon: Globe },
  ], [t, language]);
  ...
  return (
    <div className="relative">
      <AnimatePresence>
        {/* Оверлей: рендерится ТОЛЬКО когда меню открыто */}
        {isOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        {/* Боковая панель: видна если мы проскроллили ИЛИ если она уже открыта */}
        {(isVisible || isOpen) && (
          <motion.nav
          ...
          >
            ...
            <div className="flex-1 py-12 px-6 overflow-y-auto">
              <div className="mb-10 ml-4">
                <h2 className="text-[10px] text-muted-foreground uppercase tracking-[0.4em] font-bold opacity-50">
                  Navigate
                </h2>
              </div>
                ...
                {navItems.map((item, i) => {
                  ...
                  return (
                    <a
                      ...
                      href={item.href}
                      onClick={() => setIsOpen(false)}
                      className="group flex items-center gap-4 p-4 rounded-xl hover:bg-white/5 text-muted-foreground hover:text-foreground transition-all duration-300"
                    >
